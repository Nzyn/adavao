# AlertDavao Additional Testing Scenarios — Copy-Paste Tables
...
| No. | Test Case Name | Pass | Fail | Not Done |
|-----|----------------|------|------|----------|
...
| 2 | Verify that the assignment occurs automatically during report creation (ReportController store method). | | | |
...
| 3 | Verify that the AdminSide custom alert system displays icon-based dialogs (success, error, info) replacing native alerts. | | | |
